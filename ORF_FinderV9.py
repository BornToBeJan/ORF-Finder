...
def CutIntrons(Sequence):
    
    Output = ""
    
    for i in range(0, len(Sequence)): 
        # Jeder Buchstabe, der kein Kleinbuchstabe ist, wird in neuen String kopiert
        if ord(Sequence[i]) > 96 and ord(Sequence[i]) < 123:
            continue
...
###############################################################
# Funktion zum Kreieren des Reverse-Complements einer Sequenz #
###############################################################

def createReverseComplement(Sequence):
    
    # erzeugt reverse Sequenz
    RevSequ = Sequence[::-1]
     
    ComplSeq = ''
    
    # erzeugt komplementäre Sequenz zu reverser Sequenz
    for letter in RevSequ:
        if letter == 'A':
            ComplSeq += 'T'
        if letter == 'C':
            ComplSeq += 'G'
...
def writeResultsToFile(Pfad, AllOpenReadingFrames, Sequence, revSeq):
    ...
    for i in range(0, len(AllOpenReadingFrames)):
        for j in range (0, len(AllOpenReadingFrames[i])):
            ...
            AusgabeFile.write(str(AllOpenReadingFrames[i][j]['x']) + ";") #Schreiben von Startindex
            AusgabeFile.write(str(AllOpenReadingFrames[i][j]['y']) + ";") # Schreiben von Stopindex
...
# Intron Splicing
Sequence = CutIntrons(Sequence)

# Kreieren des Reverse Complements
revSeq = createReverseComplement(Sequence)
...
for i in range(0, 3):
    ...
    Temp_Liste = findOpenReadingFrame(Sequence, StopCodons, i, UserInput)
    ...
    Temp_Liste = findOpenReadingFrame(revSeq, StopCodons, i, UserInput)
...
writeResultsToFile(args.Zielpfad, AllOpenReadingFrames, Sequence, revSeq)
